...
namespace Mvvm.Messenger
{
	...
	public static class MessageProvider
	{
		...
		static readonly MessageToActionsMap invocationList = new MessageToActionsMap();
        //static readonly MessageProvider messageprovider = new MessageProvider();
		...
		public static void Register(object target)
		{
			...
			foreach (var methodInfo in target.GetType().GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
				foreach (MessageProviderSinkAttribute attribute in methodInfo.GetCustomAttributes(typeof(MessageProviderSinkAttribute), true))
        ...
        [MethodImpl(MethodImplOptions.Synchronized)] 
        public static void Register(string message, Delegate callback)
		{
        ...
        [MethodImpl(MethodImplOptions.Synchronized)] 
        public static void NotifyColleagues<T>(string message, T parameter)
		{
        ...
        /// <param name="message">The message to broadcast</param>
        [MethodImpl(MethodImplOptions.Synchronized)] 
        public static void NotifyColleagues<T>(string message)
		{
			var actions = invocationList.GetActions(message);
